...
**重要信息**：
- 输出文件名必须是：A项目第{week_num}周周报.docx（如果 task.json 中有 output_filename 字段，则使用该字段指定的文件名）
...
1. **读取任务配置**：
   ...
   ⚠️ **检查 output_filename 字段**：如果 task.json 中包含 `output_filename` 字段，则使用该字段作为输出文件名；否则使用默认命名规则。
...
6. **填充模板并导出**：
   ...
   参数：--template "{template_path}" --data "{output_path}/tmp/week_{week_num}-report.json" --output "{output_path}/[从task.json读取的output_filename或默认文件名]"

**重要提醒**：
- 必须严格遵守清洗规则，技术术语必须转换为业务语言
- 必须补充下周计划和问题章节
- ⚠️ **输出文件名规则**：
  - 优先使用 task.json 中的 `output_filename` 字段（如果存在）
  - 否则使用默认命名：`第{week_num}周周报.{format}`
